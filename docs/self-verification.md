...
The root spec (`specs/speclang.spec`) declares services for `transfer_server`, `broken_server`, and `http_test_server` in its `target` block. These containers are managed automatically by `specrun verify` via Docker.
