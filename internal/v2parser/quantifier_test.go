...
import (
	...
	"github.com/bamsammich/speclang/v2/internal/v2parser"
)
...
func TestParseAllExpr(t *testing.T) {
  ...
  scope s {
	...
	a := inv.Assertions[0]
	allExpr, ok := a.Expr.(v2parser.AllExpr)
	if !ok {
	...
	ref, ok := allExpr.Array.(v2parser.FieldRef)
	if !ok {
	...
	binOp, ok := allExpr.Predicate.(v2parser.BinaryOp)
	if !ok {
...
func TestParseAnyExpr(t *testing.T) {
  ...
  scope s {
	...
	inv := spec.Scopes[0].Invariants[0]
	anyExpr, ok := inv.Assertions[0].Expr.(v2parser.AnyExpr)
	if !ok {
...
func TestParseNestedQuantifiers(t *testing.T) {
  ...
  scope s {
	...
	inv := spec.Scopes[0].Invariants[0]
	allExpr, ok := inv.Assertions[0].Expr.(v2parser.AllExpr)
	if !ok {
	...
	anyExpr, ok := allExpr.Predicate.(v2parser.AnyExpr)
	if !ok {
